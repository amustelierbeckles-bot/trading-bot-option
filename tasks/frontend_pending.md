...
- F-2 API key en bundle (Plan A) — key eliminada del cliente. Backend: `auth_deps.verify_session_or_key` (sesión httpOnly firmada HMAC **O** X-API-Key server-side); `routes/auth.py` login/logout/me con cookie HttpOnly+Secure+SameSite=Strict; deps swapeadas en signals/trades/admin. Frontend: removido `REACT_APP_API_KEY` de dashboardUtils/useDashboard/Backtesting/ValidateMobile; `LoginGate.jsx` envuelve App. **Pendiente antes de deploy:** (1) swap `routes/risk.py` — módulo crítico, requiere frase de confirmación; (2) definir `APP_PASSWORD` en VPS `.env.production`; (3) deploy unificado (rebuild api + frontend).
